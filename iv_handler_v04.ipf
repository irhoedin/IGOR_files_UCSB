...
Menu "I-V data"
	...
	"Delete tables", kill_all_tables()
	"Delete graphs", kill_all_graphs()
	"Delete tables & graphs", kill_all_windows()
...
string windowname = id + "_g"
string grp_win_list = WinList("*_g*", ";", "")
variable grp_i = 0
string g_win_name
do
	g_win_name = stringfromlist(grp_i, grp_win_list)
	if (strlen(g_win_name)==0)
		break
	endif
	
	if (stringmatch(windowname, g_win_name)==1)
...
string photo_currlist = wavelist("*_p_*iv",";","")
...
string dark_currlist = wavelist("*_d_*iv",";","")
string dark_current
pc_i = 0
do
...
Savepict  /P=temporaryPath /E=-8  /O as savename
...
string windowname = id + "_t"

string tbl_win_list = WinList("*_t*", ";", "")
variable tbl_i = 0
string t_win_name
do
	t_win_name = stringfromlist(tbl_i, tbl_win_list)
	if (strlen(t_win_name)==0)
		break
	endif
	
	if (stringmatch(windowname, t_win_name)==1)
		killwindow $windowname
	endif
...
function kill_all_graphs()
string tbl_win_list = WinList("*_g*", ";", "")
variable tbl_i = 0
string t_win_name
do
	t_win_name = stringfromlist(tbl_i, tbl_win_list)
...
variable table = kill_all_tables()
variable graph = kill_all_graphs()
